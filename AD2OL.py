...
def parse_args():
    ...
    return (args.url, args.token, args.execute, args.groups, args.rules, args.transitiongroups, args.appmembershipgroup,
            args.apppushgroup, args.rmuserad, args.comparegroups, args.users, args.activaterules, args.deactivaterules)
...
def remove_user_from_app(application_id, user_id, **kwargs):
    return requests.delete(f'https://{url}/api/v1/apps/{application_id}/users/{user_id}', params=kwargs,
                           headers=HEADERS)
...
def deactivate_rules(**kwargs):
    for rule in get_group_rules(url):
        if 'API-AD2O' in rule['name']:
            status = requests.post(f"https://{url}/api/v1/groups/rules/{rule['id']}/lifecycle/deactivate",
...
def reset_user_password(**kwargs):
    for user in get_okta_users(url):
        response = requests.post(f"https://{url}/api/v1/users/{user['id']}/lifecycle/reset_password?sendEmail=True",
                                 params=kwargs, headers=HEADERS)
    return response
...
if __name__ == '__main__':
    ...
    if execute == False:
        ...
        print(
            f"Executing changes in your Okta instance {url}. Be sure you have done a dry run first, and compared changes. This only makes changes to the arguments you specifically want to execute. Waiting three seconds before executing changes")
        time.sleep(3)
        if groups:
            create_okta_groups()
        if rules:
        ...
        if rmuserad:
            print(
                "This is a breaking change, and you specified to remove all users from Active Directory and wipe out their password. Waiting ten seconds to confirm you wanted to make this change!")
            time.sleep(10)
